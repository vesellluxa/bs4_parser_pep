...
def pep(session):
    ...
    for pep_line in tqdm(pep_lines):
        ...
        response = get_response(session, pep_line_link)
        soup = BeautifulSoup(response.text, 'lxml')
        dl_tag = find_tag(soup, 'dl')
        status = dl_tag.find(string='Status')
        if not status:
            logging.error(f'Не найдена строка статуса! {pep_line_link}')
            continue
        ...
        if status_num not in status_ext:
            logging.info(
                ...
                f'{pep_line_link}\n'
                f'Статус в карточке - {status_num}\n'
...
def main():
    ...
    logging.info(f'Аргументы при запуске: {args}')
